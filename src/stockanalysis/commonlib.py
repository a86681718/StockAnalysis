...
from stockanalysis.config import CONF_DIR, PROJECT_ROOT
...
def _normalize_paths(section: dict) -> dict:
    normalized = dict(section)
    for key in ("root.path", "data.path", "output.path", "assets.path"):
        ...
        if not path.is_absolute():
            normalized[key] = str((PROJECT_ROOT / path).resolve())
    return normalized


def getConf(section=None):
    conf = RawConfigParser()
    conf.read(CONF_DIR / "default.properties")
    if section:
        return _normalize_paths(conf._sections.get(section, {}))
    section_override = os.getenv("STOCKANALYSIS_CONF_SECTION")
    if section_override and section_override in conf._sections:
        return _normalize_paths(conf._sections[section_override])
    if platform.system() == 'Darwin' and 'MAC' in conf._sections:
        return _normalize_paths(conf._sections['MAC'])
    if platform.system() == 'Linux' and 'GCP' in conf._sections:
        return _normalize_paths(conf._sections['GCP'])
    return _normalize_paths(conf.defaults())
