...
def wait_for_turnstile(page, max_retries=10, wait_time=5):
    ...
    :param page: DrissionPage browser page
...
def main():
    ...
    for symbol in symbols:
        ...
        while retries < 3 and not success:
            retries += 1
            logging.info("start get patched browser")
            page = get_patched_browser(headless=False)
            logging.info("finish get patched browser")

            try:
                # Open target page
                logging.info("start get tpex page")
                page.get("https://www.tpex.org.tw/zh-tw/mainboard/trading/info/brokerBS.html")
                logging.info("finish get tpex page")
