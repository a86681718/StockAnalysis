    ...
    try:
        for idx, symbol in enumerate(symbols, 1):
            ...
            while retries < 3 and not success:
                ...
                if not token:
                    logging.error(f"Failed to obtain Turnstile token for symbol {symbol} (attempt {retries}/3)")
                    continue

                success = crawl_stock_data(symbol, token, output_dir)
                if not success:
                    logging.warning(f"Crawl failed for symbol {symbol} (attempt {retries}/3)")
                    time.sleep(1)
