...
from stockanalysis.config import ensure_dir, resolve_assets, resolve_data, resolve_output
...
def solve_captcha(source_img_path, model):
    ...
    processed_img_path = str(CAPTCHA_DIR / "preprocessing.jpg")
    ...
    for stock_code in stock_list:
        ...
        while failed:
            try:
                ...
                captcha_tmp_path = CAPTCHA_DIR / "tmp.png"
                with open(captcha_tmp_path, 'wb') as f:
                    shutil.copyfileobj(img.raw, f)

                captcha_num = solve_captcha(str(captcha_tmp_path), model)
                if not captcha_num:
                    failed_count += 1
                ...
                output_path = BS_DATA_DIR / data_dt / "twse" / f"{stock_code}.csv"
...
if __name__ == "__main__":
    ...
    stock_list = get_stock_list(data_dt) 
    if not stock_list:
        logging.warning(f"No stocks found for date: {data_dt}")
        sys.exit(0)
    logging.info(f"Total stocks found: {len(stock_list)}")  
    warrant_list = get_warrant_list(data_dt)
    if not warrant_list:
        logging.warning(f"No warrants found for date: {data_dt}")
    else:
        logging.info(f"Total warrants found: {len(warrant_list)}")
        stock_list.extend(warrant_list)

    # Keep track of crawled stocks
    logging.info("Check if any stocks/warrants had been crawled")
    data_path = BS_DATA_DIR / data_dt
    twse_dir = data_path / "twse"
    twse_dir.mkdir(parents=True, exist_ok=True)
    crawled_list = [file_name.replace('.csv', '') for file_name in os.listdir(twse_dir) if '.csv' in file_name]
