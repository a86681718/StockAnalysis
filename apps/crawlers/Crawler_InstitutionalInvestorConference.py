...
root_path = ensure_dir(resolve_data("institutionalInvestorConference"))
...
for year in pdf['年份'].unique():
    ...
    if file_path.exists():
        old_pdf = pd.read_csv(file_path)
        old_pdf['股票代號'] = old_pdf['股票代號'].map(lambda x: str(x))
        new_pdf = old_pdf.append(tmp_pdf).drop_duplicates()
        new_pdf.sort_values(['法說會日期', '股票代號']).to_csv(file_path, index=False)
    else:
        tmp_pdf.sort_values(['法說會日期', '股票代號']).to_csv(file_path, index=False)
