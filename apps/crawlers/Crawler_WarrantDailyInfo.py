...
output_path = ensure_dir(resolve_data("warrant"))
fileName = f'warrant_raw_{date}'
extension = '.xls'
headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36'}
resp = requests.get(url, headers=headers)
if resp.status_code == 200:
    with open(output_path / f"{fileName}{extension}", "wb") as f:
        for chunk in resp.iter_content(chunk_size=8192):
            f.write(chunk)

    # extract information to new CSV file
    pdf = pd.read_excel(output_path / f"{fileName}{extension}", sheet_name='summary', skiprows=4)
    ...
    extension = '.csv'
    pdf.to_csv(output_path / f"{fileName.replace('_raw', '')}{extension}", index=False)
else:
