...
| Broker-report ETL | production local operation | `uv run apps/etl/run_bs_report_etl.py --market ...` | `src/stockanalysis/pipelines/broker_reports.py` | GCS/local dated CSV folders and manifest | per-symbol Parquet, manifest, archive | `apps/etl/README.md` |
| OHLC ETL | production local operation | `uv run apps/etl/build_ohlc_parquet.py` | `src/stockanalysis/pipelines/ohlc.py` | TWSE/TPEX daily OHLC CSV | `data/_derived/ohlc.parquet` | current data layout and Dash consumer |
