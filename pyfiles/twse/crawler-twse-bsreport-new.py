...
def build_paths(data_dt: str) -> CrawlerPaths:
    img_dir = os.path.join(ROOT_PATH, IMG_SUBFOLDER)
    data_dir = os.path.join(ROOT_PATH, DATA_SUBFOLDER)
    report_dir = os.path.join(data_dir, 'bs_data', data_dt, 'twse')
    return CrawlerPaths(root=ROOT_PATH, img_dir=img_dir, data_dir=data_dir, report_dir=report_dir)
...
def load_captcha_model(paths: CrawlerPaths):
    model_path = os.path.join(paths.data_dir, "twse_cnn_model.hdf5")
